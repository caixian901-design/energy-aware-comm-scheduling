...
kref = 8
cref = [xining_run(0.80, kref, "qinghai", h)[3] for h in hours]
cmn, cmx = min(cref), max(cref)
ax.text(0.97, 0.04, f"fixed schedule k = {kref}: best hour cuts carbon "
        f"{(cmx-cmn)/cmx*100:.0f}% vs worst", transform=ax.transAxes,
        ha="right", fontsize=6.5, color="#555")
